...
export default function Account() {
  const [user, setUser] = useState<null | UserExtended>(null);
  ...
  useEffect(() => {
    const getUser = async () => {
      ...
      if (data.session) {
        // Fetch the user details including `is_eligible_for_free_pass`
        ...
        if (error) {
          ...
          setUser({
            ...data.session.user,
            is_eligible_for_free_pass: userData.is_eligible_for_free_pass,
          });
          setIsEligibleForFreePass(userData.is_eligible_for_free_pass);
        }
      } else {
  ...
  useEffect(() => {
    ...
    switch (tabInParam) {
      ...
      case "my-ticket":
        ...
        break;
      case "my-payment":
        if (!isEligibleForFreePass) {
          setActiveTab(2);
        } else {
          setActiveTab(0); // Redirect to default tab
        }
        break;
  ...
  }, [tabInParam, user, isEligibleForFreePass]);
  ...
  const changeTab = (tab: 0 | 1 | 2 | 3) => {
    if (tab === 2 && isEligibleForFreePass) {
      return;
    }
    const param =
      tab === 0
  ...
  return (
    <div className="p-4 md:px-16 md:py-24 flex flex-col md:flex-row w-full">
      <aside className="flex flex-col w-full md:w-1/4 lg:w-1/6 space-y-4 p-4 md:border-r border-slate-400">
        <button
          className={cn(
      ...
      <div className="flex-grow p-4">
        ...
        {activeTab === 2 && !isEligibleForFreePass && (
